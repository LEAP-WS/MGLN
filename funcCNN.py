...
def Label2D(label_vec, num_classes):
    num1 = np.shape(label_vec)[0]
    label_mat = np.zeros([num1, num_classes])
    if np.size(np.shape(label_vec)) == 2:
        label_vec = np.matlib.reshape(label_vec, num1)
    pos_x = [i for i in range(num1)]
    label_mat[pos_x, label_vec] = 1
    return label_mat
...
def Normalization_2dMat(mat1):
    for col_idx in range(np.shape(mat1)[1]):
        max_val = np.max(mat1[:, col_idx])
        min_val = np.min(mat1[:, col_idx])
        mat1[:, col_idx] = (mat1[:, col_idx]-min_val)/(max_val-min_val)
    return mat1

def FindQDifference(Q1, Q2):
    max1 = np.argmax(Q1) 
    max2 = np.argmax(Q2)
    return [max1, max2]

def Assign2Gt(pred_lab, tepos2d, gt):
    gt1 = gt.copy()
    tenum = np.shape(tepos2d)[0]
    trnum = np.shape(pred_lab)[0]-tenum
    
    gt1[tepos2d[:, 0], tepos2d[:, 1]] = pred_lab[trnum:trnum+tenum]
    return gt1

def Cal01DisTrainMat(trmask, lab_vec, full_diag): 
    sp_num = np.size(trmask)
    Mat01 = np.zeros([sp_num, sp_num])
    Mat01_tr = np.zeros([sp_num, sp_num])
    te_pos = np.argwhere(trmask == 0)
    lab_vec[te_pos[:,0]] = -1
    max_lab = int(np.max(lab_vec))
    for lab_idx0 in range(max_lab):
        pos1 = np.argwhere(lab_vec == lab_idx0)
        pos2 = np.argwhere(lab_vec >= 0)
        for pos0 in list(pos1):
            Mat01[np.array([pos0 for i in range(np.size(pos1))]), pos1] = 1
            Mat01_tr[np.array([pos0 for i in range(np.size(pos2))]), pos2] = 1
    if full_diag == True:
        Mat01[np.array([i for i in range(sp_num)]), np.array([i for i in range(sp_num)])] = 1
        Mat01_tr[np.array([i for i in range(sp_num)]), np.array([i for i in range(sp_num)])] = 1
    elif full_diag == 'zero':
        Mat01[np.array([i for i in range(sp_num)]), np.array([i for i in range(sp_num)])] = 0
        Mat01_tr[np.array([i for i in range(sp_num)]), np.array([i for i in range(sp_num)])] = 0
    
    return Mat01, Mat01_tr

def KnnA(A, k):
    num_row = np.shape(A)[0]
    for row_idx0 in range(num_row):
        kmax_vals = np.sort(A[row_idx0, :])[::-1] 
        threshold_val = kmax_vals[k-1]
        pos1 = np.argwhere(A[0, :]<threshold_val)
        A[row_idx0, pos1] = 0.0
    return A

def MatPower(mat1, p):
    [D, V] = np.linalg.eig(mat1)
    Dp = np.power(D, p)
    FH = np.matmul(np.matmul(V, np.diag(Dp)), V.T)
    return 0.5*(FH + FH.T)
